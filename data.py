...
# ------------------------------------------------------------------------------------------------------------------------------
	...
	#The default label is volumetric_soil_water_layer_20
        ...
        mask = np.ones(label[0,:,:,:].shape)
        mask[np.isnan(label[0,:,:,0])] = 0
        mask[np.isnan(label[0,:,:,1])] = 0
        ...
        if not os.path.exists(PATH + 'static_norm.npy'):
            ...
            for i in range(len(self.static_list)):
                ...
                with xr.open_dataset(file_static) as f_:
                    ...
                    static_data = self._interp(cfg,static_data,mask)
                static.append(static_data)
            static = np.stack(static, axis=-1)  #  n 45 90 1
            print("static shape is ",static.shape)
            if cfg['data_type']=='float32':
                static = static.astype(np.float32)
            static = self._spatial_normalize(cfg,static)
            for i in range(cfg["num_repeat"]):
                noise_ = self.generate_custom_perlin_noise(static[i].shape,flag=False)
                static[i] = noise_ + static[i]
            print('static type is {d_p}'.format(d_p=static[0].dtype))
            with open(PATH + 'static_norm.npy', 'wb') as f:
...
# ------------------------------------------------------------------------------------------------------------------------------
	#Partition and create datasets
        ...
        print('land_surface shape is {shape}'.format(shape=land_surface.shape))
        print('land surface shape is {shape}'.format(shape=land_surface.shape))
        print('label shape is {shape}'.format(shape=label.shape))
        print('static shape is {shape}'.format(shape=static[0].shape))
...
# save
	    #Save the normalized dataset
            ...
            noise_ = self.generate_custom_perlin_noise(x_train.shape,flag=True)

            x_train = noise_ + x_train
...
#------------------------------------------------------------------------------------------------------------------------------
    def _spatial_normalize(self,cfg, static):
...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    def generate_custom_perlin_noise(self, size, octaves=4, scaling_factor=0.5, periods=(6, 6, 1),flag= False):
        """
        生成定制的Perlin噪声。
        """
        if flag:
            no_days, latitudes, longitudes, features = size
        else:latitudes, longitudes, features = size
        lat_period, lon_period, ch_period = periods

        # 只为一天生成噪声数据
        noise_data_day = np.zeros((latitudes, longitudes, features), dtype=np.float32)

        for lat in range(latitudes):  # 对于每个纬度点
            for lon in range(longitudes):  # 对于每个经度点
                for ch in range(features):  # 对于每个特征（通道）
                    # 计算噪声函数中的实际位置
                    x = lat / lat_period
                    y = lon / lon_period
                    z = ch / ch_period
                    ...
                    # 生成并应用噪声
                    noise_value = pnoise3(x, y, z, octaves=octaves) * scaling_factor
                    noise_data_day[lat, lon, ch] = noise_value

        # 通过复制一天的数据生成所有天数的噪声数据
        if flag:
            noise_data = np.repeat(noise_data_day[np.newaxis, :, :, :], no_days, axis=0)
            return noise_data
        else:
            return noise_data_day
